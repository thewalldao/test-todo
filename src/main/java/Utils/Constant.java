package Utils;

public class Constant {
    public static final int LONG_WAIT_TIMEOUT = 40;
    public static final int WAIT_TIMEOUT = 20;
    public static final int WAIT_MEDIUM_TIMEOUT = 10;
    public static final int WAIT_SHORT_TIMEOUT = 5;
    public static final String ALLURE_RESULT_PATH = System.getProperty("user.dir") + "/allure-results";
    public static final String HUB_URL = "http://localhost:4444/wd/hub";
    public static final String AUT_URL = "https://tiki.vn";
    public static final String DEFAULT_BROWSER = "chrome";
    public static final String DEFAULT_REMOTE_STATE = "false";
    public static final String DEFAULT_GITHUB_USERNAME = "tuan041.test.test@gmail.com";
    public static final String DEFAULT_GITHUB_PASSWORD = "Ttuan041";

}
